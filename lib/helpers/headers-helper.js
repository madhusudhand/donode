...
class HeadersHelper {
  ...
  */
  getRouteHeaders(route, appHeaders) {
    ...
    headers = headers.concat(route.headers.all).concat(route.headers.only);
  ...
  */
  _validateHeaders(route) {
    if (!route.headers) {
      route.headers = { all: [], only: [], children: [] };
      return;
    }

    if (Array.isArray(route.headers)) { // is array?
      // attach them to current route and its children
      route.headers = { all: route.headers, only: [], children: [] };

    } else if (typeof route.headers === 'object') { // is object?

      // fill empty if not defined.
      route.headers.all = route.headers.all || [];
      route.headers.only = route.headers.only || [];
      route.headers.children = route.headers.children || [];

      // validate if all, only, children are arrays
      if (
        !Array.isArray(route.headers.all) ||
        !Array.isArray(route.headers.only) ||
