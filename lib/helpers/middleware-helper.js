...
class MiddlewareHelper {
  ...
  requireMiddleware(options) {
    this._middlewareClasses = requireDir(module, options.middlewarePath);
  }
  ...
  _getParentMiddleware(route) {
    ...
    function collect() {
      ...
      if (typeof route.middleware === 'object') {
        let childMiddleware = [];
        if (route.middleware.all && Array.isArray(route.middleware.all))
          childMiddleware = childMiddleware.concat(route.middleware.all);
        if (route.middleware.children && Array.isArray(route.middleware.children))
          childMiddleware = childMiddleware.concat(route.middleware.children);
