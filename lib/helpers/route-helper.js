...
class RouteHelper {
  ...
  */
  collectRoutes(rawRoutes, config) {
    ...
    this._processRawRoutes(rawRoutes);
    return this.routes;
  }
  ...
  **
  */
  _processRawRoutes(rawRoutes, parent) {
    ...
    for (let route of rawRoutes) {
      ...
      if (Array.isArray(route.children)) {
        this._processRawRoutes(route.children, route);
      }
    }
  ...
  */
  _processRoute(route) {
    ...
    const routePath = this._removeDupeSlashes(this._getRoutePath(route));
    // route params
    const params = paramHelper.parseParams(routePath);
    ...
    return {
      ...
      headers: headersHelper.getRouteHeaders(route, appHeaders),
    };
  }
  ...
  **       on: BOOTSTRAP
  **
  **  remove extra slashes in the path and trailing slash.
  **
  */
  _removeDupeSlashes(routePath) {
