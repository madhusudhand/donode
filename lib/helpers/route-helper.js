...
class RouteHelper {
  ...
  */
  collectRoutes(routeConfig, options) {
    ...
    this.options = Object.assign({}, options, {
      relControllerPath: path.join(options.appDir, options.controllerDir)
    });

    // collect all controllers from the given controllerDir
    this._controllerClasses = this._loadControllers(options.controllerPath);
  ...
  */
  _processRoute(route) {
    ...
    if (!this._controllers[controllerKey]) {
      ...
      if (handler.controllerPath) {
        ...
        for (let key of handler.controllerPath.split(path.delimiter)) {
          ...
          if (!ctrlClasses) {
            errorHelper.throwError({
              error: `directory '${handler.controllerPath}' not found in '${this.options.relControllerPath}'`,
              line : `Route: { path: ${route.path}, method: ${route.method}, handler: ${route.handler} }`,
              file : `routes.js`,
              hint : `make sure path is correct (case sensitive) and is present in ${this.options.relControllerPath}`
            });
          }
        }
      } else {
      ...
      if (!ControllerClass) {
        errorHelper.throwError({
          ...
          line : `Route: { path: ${route.path}, method: ${route.method}, handler: ${route.handler} }`,
          file : `routes.js`,
          hint : [`make sure Controller exists in ${path.join(this.options.relControllerPath, handler.controllerPath)}.`,
    ...
    try {
      // check if the handler method defined
      const ctrlObj = new this._controllers[controllerKey]();
      handlerMethod = ctrlObj[handler.methodName];
    } catch (e) {
      ...
      errorHelper.throwError({
        ...
        line : `Route: { path: ${route.path}, method: ${route.method}, handler: ${route.handler} }`,
        file : `${path.join(this.options.relControllerPath, controllerKey)}.js`,
        hint : `A controller is a class and should be exported. module.exports = ClassName;`
      });
    }
